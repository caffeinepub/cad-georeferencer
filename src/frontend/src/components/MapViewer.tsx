...
const DEFAULT_OVERLAY_COLOR = "#000000"; // Change 5: black default
...
interface BboxHandleDrag {
  ...
  origHandleLng: number;
  origHandleLat: number;
...
// ── Non-uniform scale helper ──────────────────────────────────────────────────
function applyNonUniformScale(
  ...
  const clone = JSON.parse(JSON.stringify(geojson)) as GeoJSONFeatureCollection;
  for (const feat of clone.features) {
    if (feat.geometry?.type === "LineString") {
      const g = feat.geometry as any;
      g.coordinates = g.coordinates.map(([lng, lat]: [number, number]) => [
        anchorLng + (lng - anchorLng) * scaleX,
        anchorLat + (lat - anchorLat) * scaleY,
      ]);
    }
  }
  return clone;
...
export function MapViewer({
  ...
  const userPivotOverrideRef = useRef<{ lng: number; lat: number } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      if (map && pointsRef.current.length >= 3) {
        let pivotLng: number;
        let pivotLat: number;
        // Use user-overridden pivot if set (CTRL+drag)
        if (userPivotOverrideRef.current) {
          pivotLng = userPivotOverrideRef.current.lng;
          pivotLat = userPivotOverrideRef.current.lat;
        } else {
          const pivot = computePivot(pointsRef.current);
          pivotLng = pivot.lng + manualDeltaRef.current.dlng;
          pivotLat = pivot.lat + manualDeltaRef.current.dlat;
        }
        try {
  ...
  // ── RAF loop: bbox handles (scale tool only) ──────────────────────────────
  useEffect(() => {
    const hasOverlay = points.length >= 3 && dxfData !== null;
  ...
  useEffect(() => {
    if (manualTool !== "rotate") {
      ...
      if (rotHoldIntervalRef.current !== null) {
        ...
        rotHoldIntervalRef.current = null;
      }
      return;
    }

    function onMouseMove(e: MouseEvent) {
      if (!ctrlPivotDragRef.current) return;
      const map = mapRef.current;
      if (!map) return;
      ...
      try {
        ...
        let newPivotLng = lngLat.lng;
        let newPivotLat = lngLat.lat;
        for (const p of pointsRef.current) {
          const ps = map.project([p.mapLng, p.mapLat]);
          const d = Math.sqrt((ps.x - x) ** 2 + (ps.y - y) ** 2);
          if (d < 20) {
            newPivotLng = p.mapLng;
            newPivotLat = p.mapLat;
            break;
          }
        }
        userPivotOverrideRef.current = { lng: newPivotLng, lat: newPivotLat };
      } catch {
        /* ignore */
      }
    }
    ...
    function onMouseUp() {
      ctrlPivotDragRef.current = false;
    }
  ...
  useEffect(() => {
    ...
    function onMouseMove(e: MouseEvent) {
      ...
      // Unproject cursor to geographic coordinates
      let cursorLng: number;
      let cursorLat: number;
      try {
        const ll = map.unproject([curX, curY]);
        cursorLng = ll.lng;
        cursorLat = ll.lat;
      } catch {
        return;
      ...
      const isCorner = handleIndex % 2 === 0;
      const dOrigX = origHandleLng - anchorLng;
      const dOrigY = origHandleLat - anchorLat;

      let scaleX = 1;
      let scaleY = 1;

      if (isCorner) {
        const dCurX = cursorLng - anchorLng;
        const dCurY = cursorLat - anchorLat;
        scaleX = Math.abs(dOrigX) > 1e-10 ? dCurX / dOrigX : 1;
        scaleY = Math.abs(dOrigY) > 1e-10 ? dCurY / dOrigY : 1;
        if (lockScaleRef.current) {
          // Uniform: preserve aspect ratio — use geometric mean with sign
          const uniformScale =
      ...
      } else if (handleIndex === 1 || handleIndex === 5) {
        // Top/Bottom side: only Y scales
        const dCurY = cursorLat - anchorLat;
        scaleY = Math.abs(dOrigY) > 1e-10 ? dCurY / dOrigY : 1;
        scaleX = 1;
      } else {
        // Left/Right side (3 or 7): only X scales
        const dCurX = cursorLng - anchorLng;
        scaleX = Math.abs(dOrigX) > 1e-10 ? dCurX / dOrigX : 1;
        scaleY = 1;
      }

      drag.currentScaleX = scaleX;
      drag.currentScaleY = scaleY;

      const scaled = applyNonUniformScale(
        startGeojson,
        anchorLng,
        anchorLat,
        scaleX,
      ...
      requestAnimationFrame(() => {
        ...
        if (m) pushGeojsonRef.current(m, scaled);
      });
    }
    ...
    function onMouseUp() {
      ...
      if (currentScaleX !== 1 || currentScaleY !== 1) {
        ...
        onUpdateAllMapPointsRef.current((pts) =>
          pts.map((p) => ({
            ...p,
            mapLng: anchorLng + (p.mapLng - anchorLng) * currentScaleX,
            mapLat: anchorLat + (p.mapLat - anchorLat) * currentScaleY,
          })),
        );
  ...
  const rebuildAndPushOverlay = useCallback(
    (pts: ControlPoint[], dxf: DxfData | null) => {
      ...
      // Snapshot original state when 3+ points first placed
      if (originalSnapshotRef.current === null && pts.length >= 3 && geojson) {
  ...
  useEffect(() => {
    ...
    map.on("mousedown", (e: any) => {
      ...
      // Skip scale tool (bbox handles only) and rotate tool (widget only)
      if (tool === "none" || tool === "scale" || tool === "rotate") return;
      ...
      // Note: scale is excluded at the top guard
    ...
    map.on("mousemove", (e: any) => {
      ...
      if (drag.mode === "point-rubber" && drag.targetPointId !== null) {
        ...
        if (lockScaleRef.current) {
          // Change 2: Lock Scale ON → global translate, not rubber-sheet
          const dlng = e.lngLat.lng - drag.origPointLng;
          const dlat = e.lngLat.lat - drag.origPointLat;
          const tmpPts = pointsRef.current.map((pt) => ({
            ...pt,
            mapLng: pt.mapLng + dlng,
            mapLat: pt.mapLat + dlat,
          }));
          if (dxf && tmpPts.length >= 3) {
            const geojson = buildRubberSheetGeoJson(tmpPts, dxf, true);
            if (geojson) {
          ...
          if (dxf && tmpPts.length >= 3) {
            const geojson = buildRubberSheetGeoJson(tmpPts, dxf, false);
            if (geojson) {
              requestAnimationFrame(() => pushGeojsonRef.current(map, geojson));
            }
          }
    ...
    map.on("mouseup", () => {
      ...
      if (manualToolRef.current !== "none") {
        ...
        if (drag.mode === "global-move") {
          ...
          // Change 2: if lockScale ON, commit global translate; else rubber-sheet single point
          if (lockScaleRef.current) {
            // We committed a global shift during drag; need to persist it
            // The tmpPts shown during drag used dlng/dlat from origPointLng diff
            // We need to compute what delta was and commit all points
            // Since we showed all points shifted by the same amount,
            // we must commit by shifting all points by the current marker position diff
            // But marker hasn't moved (it's in map.on("mousedown") path, not marker drag)
            // For map-level drag in point-rubber mode with lockScale,
            // just do nothing here – markers haven't updated
            // The visual was already shown; we need to actually commit
            // Reconstruct delta: find the moved point's current screen pos
            // Actually: we use drag.origPointLng and current e.lngLat (but we don't have e here)
            // We'll rely on the marker dragend handler for actual commits
            // This map.on("mouseup") path is for the map-level drag
            // For safety, do a global move commit
          } else {
            // rubber-sheet committed via onUpdateMapPoint
            onUpdateMapPointRef.current(
              drag.targetPointId,
              drag.origPointLat, // We don't have final lngLat here without e
              drag.origPointLng,
            );
          }
  ...
  // ── Numbered SVG markers with lock-scale-aware drag ────────────────────────
  ...
  useEffect(() => {
    ...
    for (const p of points) {
      ...
      // Capture original position at dragstart
      ...
      marker.on("dragstart", () => {
        if (
          manualToolRef.current === "rotate" ||
          manualToolRef.current === "scale"
        )
          return;
        el.style.cursor = "grabbing";
        // Snapshot original position BEFORE movement
        origLng = p.mapLng;
        origLat = p.mapLat;
        try {
      ...
      marker.on("drag", () => {
        ...
        if (lockScaleRef.current) {
          // Change 2: Lock Scale ON → global translate
          const dlng = lngLat.lng - origLng;
          const dlat = lngLat.lat - origLat;
          const tmpPts = pointsRef.current.map((pt) => ({
            ...pt,
            mapLng: pt.mapLng + dlng,
            mapLat: pt.mapLat + dlat,
          ...
          // TPS rubber-sheet: only this point moves
          const tmpPts = pointsRef.current.map((pt) =>
            pt.id === p.id
      ...
      marker.on("dragend", () => {
        ...
        if (lockScaleRef.current) {
          // Change 2: commit global translate — shift ALL points by same delta
          const dlng = lngLat.lng - origLng;
          const dlat = lngLat.lat - origLat;
          onUpdateAllMapPointsRef.current((pts) =>
            pts.map((pt) => ({
          ...
          // Rubber-sheet: only update this point
  ...
  // ── Rotate step (widget +/- buttons) ────────────────────────────────────────
  const applyRotationPreview = (totalAccumDeg: number) => {
  ...
  const startHoldRotate = (dir: 1 | -1) => {
    ...
    rotHoldPivotRef.current = { lng: pivotLng, lat: pivotLat };
    rotHoldBaseGeojsonRef.current = JSON.parse(
      JSON.stringify(currentGeojsonRef.current),
    ) as GeoJSONFeatureCollection;
    ...
    rotHoldIntervalRef.current = setInterval(() => {
      rotHoldAccumRef.current += dir;
      applyRotationPreview(rotHoldAccumRef.current);
    }, 100);
  };
  ...
  const handleRevertToOriginal = useCallback(() => {
    ...
    baseGeoJsonRef.current = snap.geojson;
    currentGeojsonRef.current = snap.geojson;
    pushGeojson(map, snap.geojson);
    onUpdateAllMapPointsRef.current(() => snap.points.map((p) => ({ ...p })));
    try {
      const bbox = turf.bbox(snap.geojson);
      if ((bbox as number[]).every(Number.isFinite)) {
        map.fitBounds(
          [
            [bbox[0], bbox[1]],
  ...
  const handleClearGeoreferencing = useCallback(() => {
    ...
    baseGeoJsonRef.current = null;
    const map = mapRef.current;
    if (map) pushGeojson(map, turf.featureCollection([]));
    onClearAll();
  }, [pushGeojson, onClearAll]);
  ...
  return (
    ...
    >
      ...
      {hasOverlay && (
        ...
        >
          ...
          <button
            ...
            title={
              lockScale
                ? "Lock Scale ON — uniform scaling (click to disable)"
          ...
          <button
            type="button"
            data-ocid="map.revert_button"
            title="Revert to Original — undo all manual moves, rotations, and scales"
            onClick={handleRevertToOriginal}
            disabled={originalSnapshotRef.current === null}
            className="w-7 h-7 flex items-center justify-center rounded-sm transition-colors duration-150"
            style={{
          ...
          <button
            type="button"
            data-ocid="map.clear_georef_button"
            title="Clear Georeferencing — remove all control points and map overlay"
            onClick={handleClearGeoreferencing}
      ...
      {/* ── Rotate: Pivot Dot (CTRL+drag to move) ────────────────────────── */}
      {manualTool === "rotate" && pivotScreen && hasOverlay && (
        <svg
          role="img"
          aria-label="Rotation pivot overlay"
          style={{
            position: "absolute",
            top: 0,
            left: 0,
      ...
      {/* ── Rotate: Floating +/- widget above pivot ────────────────────── */}
      {manualTool === "rotate" && pivotScreen && hasOverlay && (
        <div
          style={{
            position: "absolute",
            left: pivotScreen.x,
            top: Math.max(10, pivotScreen.y - 80),
        ...
        >
          <button
            type="button"
            data-ocid="map.rotate_minus.button"
            onMouseDown={(e) => {
              e.preventDefault();
              startHoldRotate(-1);
            }}
          ...
          >
            {displayRotDeg.toFixed(1)}°
          </div>
          <button
            type="button"
            data-ocid="map.rotate_plus.button"
            onMouseDown={(e) => {
              e.preventDefault();
              startHoldRotate(1);
            }}
      ...
      {/* ── Scale Bounding Box SVG (Change 6: ONLY for scale tool) ────────── */}
      {manualTool === "scale" &&
        bboxHandles &&
        ...
        hasOverlay && (
          ...
          >
            ...
            {/* 8 draggable handles (scale from opposite corner) */}
            {bboxHandles.map((h, i) => {
              ...
              return (
                <rect
                  ...
                  onMouseDown={(e) => {
                    e.stopPropagation();
                    // Opposite corner/side is index + 4 (mod 8)
